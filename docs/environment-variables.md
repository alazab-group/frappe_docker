...
- `FRAPPE_SITE_NAME_HEADER`: Set proxy header `X-Frappe-Site-Name` and serve site named in the header, defaults to `$host`, i.e. find site name from host header. More details [below](#alazab_site_name_header)
...
Frappe framework release. You can find all releases [here](https://github.com/alazab-group/frappe/releases).
...
ERPNext [release](https://github.com/alazab-group/erpnext/releases). This variable is required if you use ERPNext override.

### `LETSENCRYPT_EMAIL`

Email that used to register https certificate. This one is required only if you use HTTPS override.

### `ALAZAB_SITE_NAME_HEADER`
